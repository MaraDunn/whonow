-- Add preferred_contact_interval_days and client_weight to list_contacts_slim
-- so the grid can compute correct relationship health scores on first load
-- (without these, all clients default to 30-day interval and show "70 Healthy" until clicked).
...
COMMENT ON FUNCTION public.list_contacts_slim IS 'Slim contact list for grid; includes city/state/address and preferred_contact_interval_days/client_weight for correct health scores on load.';
