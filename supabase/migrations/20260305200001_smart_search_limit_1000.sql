-- Allow smart_search_contacts to return up to 1000 results for smart folder (bookmarked search) view.
-- Only change: lim n from 100 to 1000. Search bar still uses _limit 10; folder view uses _limit 500.
DROP FUNCTION IF EXISTS public.smart_search_contacts(uuid, text, timestamptz, timestamptz, timestamptz, timestamptz, text, text, text[], text, text, text, int);
...
AS $$
  ...
  filtered AS (
    ...
    WHERE c.deleted_at IS NULL
      ...
      AND (
        _job_title IS NULL
        OR c.role ILIKE '%' || _job_title || '%' OR c.company ILIKE '%' || _job_title || '%' OR _job_title = ANY(c.tags)
        OR (_semantic_hint IS NOT NULL AND _semantic_hint != '' AND (c.role ILIKE '%' || _semantic_hint || '%' OR c.company ILIKE '%' || _semantic_hint || '%' OR _semantic_hint = ANY(COALESCE(c.tags, '{}'))))
      )
      ...
      AND (_company IS NULL OR c.company ILIKE '%' || _company || '%')
...
$$;
GRANT EXECUTE ON FUNCTION public.smart_search_contacts(uuid, text, timestamptz, timestamptz, timestamptz, timestamptz, text, text, text[], text, text, text, int) TO authenticated;
COMMENT ON FUNCTION public.smart_search_contacts IS 'Smart search: FTS for ranking only. Limit 1-1000 (search bar=10, smart folder=500).';
