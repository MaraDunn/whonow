-- Add last_contacted_at filter to smart_search_contacts for "who did I call last week" queries
-- Drop the previous overload (11 args) so we replace it with the new 13-arg version.

DROP FUNCTION IF EXISTS public.smart_search_contacts(
...
AS $$
  ...
  filtered AS (
    ...
    WHERE c.deleted_at IS NULL
      ...
      -- Last contacted filter: "who did I call last week"
      AND (
...
GRANT EXECUTE ON FUNCTION public.smart_search_contacts(
  UUID, text, timestamptz, timestamptz, timestamptz, timestamptz, text, text, text[], text, text, text, int
) TO authenticated;

COMMENT ON FUNCTION public.smart_search_contacts IS 
'Universal smart search: responsibility matching, created_at and last_contacted_at time filters, text search, location.';
