-- Add _client_only and _shared_only to smart_search_contacts so client directory and team directory
-- smart folders can restrict results to clients only (is_client = true) or shared only (is_shared = true).
...
COMMENT ON FUNCTION public.smart_search_contacts IS 'Smart search: FTS for ranking only. _client_only and _shared_only restrict to clients or shared contacts (for client/team directory smart folders). Limit 1-1000.';
