-- Add city, state, and address to list_contacts_slim so smart folder location filters work.
-- Without these, client-side filtering (applySearchFiltersToContacts) never sees location data.
...
COMMENT ON FUNCTION public.list_contacts_slim IS 'Slim contact list for grid; includes city/state/address for smart folder location filtering.';
