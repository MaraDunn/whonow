...
import { Users, TrendingUp, AlertCircle, Heart, Expand } from "lucide-react";
import { useOrgRelationshipInsights } from "@/hooks/useOrgRelationshipInsights";
import { useReminderSettings } from "@/hooks/useReminderSettings";
import { Skeleton } from "@/components/ui/skeleton";
import { cn } from "@/lib/utils";
import {
  OrgAddedContactsDialog,
  OrgContactedContactsDialog,
  OrgStaleContactsDialog,
  OrgHealthScoreDialog,
  OrgExpandedChartDialog,
} from "@/components/dashboard/OrgInsightDialogs";
...
function GrowthChart({ data }: GrowthChartProps) {
  ...
  return (
    <div>
      ...
      >
        <defs>
          <linearGradient id="org-growth-fill" x1="0" y1="0" x2="0" y2="1">
...
interface OrgRelationshipInsightsCardProps {
  reminderInterval?: number;
}

export function OrgRelationshipInsightsCard({
  reminderInterval = 30,
}: OrgRelationshipInsightsCardProps) {
  const { data: metrics, isLoading } = useOrgRelationshipInsights(reminderInterval);
      ...
      <div className="flex items-start justify-between gap-4 mb-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Overview of your organization's shared contact activity this month
          </p>
        </div>
      </div>
