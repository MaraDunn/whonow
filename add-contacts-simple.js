/**
 * SIMPLE BROWSER VERSION - Add 100 Test Contacts
 * 
 * Instructions:
 * 1. Sign in to your app at http://localhost:8080
 * 2. Open browser console (F12 → Console tab)
 * 3. Copy the SUPABASE_URL and SUPABASE_KEY from your .env file
 * 4. Paste this code and replace the values below
 * 5. Press Enter
 */

// ⚠️ UPDATE THESE VALUES FROM YOUR .env FILE ⚠️
...
(async function() {
  ...
  const companies = ['TechCorp Solutions', 'CloudBase Inc', 'DataSync Technologies', 'ByteForge Systems', 'QuantumSoft Labs', 'FinancePlus Holdings', 'Capital Partners Group', 'Apex Investments LLC', 'Sterling Bank & Trust', 'HealthTech Medical', 'MedCore Systems', 'Wellness Labs Inc', 'BioGenix Research', 'DesignLab Creative', 'MediaHub Productions', 'Creative Studios Agency', 'BrandWorks Marketing', 'Legal Partners LLP', 'Consulting Group International', 'HR Solutions Corp', 'Logistics Pro Services', 'Global Trade Enterprises', 'Precision Manufacturing', 'EcoGreen Industries', 'NextGen Robotics'];
  const roles = ['Software Engineer', 'Product Manager', 'Marketing Director', 'Sales Manager', 'HR Manager', 'CEO', 'CTO', 'CFO', 'VP of Engineering', 'VP of Sales', 'Design Director', 'Operations Manager', 'Business Analyst', 'Data Scientist', 'UX Designer', 'Account Executive', 'Customer Success Manager', 'Financial Analyst', 'HR Business Partner', 'Legal Counsel', 'Project Manager', 'Content Strategist'];
  ...
  // Get user's profile and folders
  const { data: profile } = await supabase.from('profiles').select('company_id').eq('id', userId).single();
  const companyId = profile?.company_id || null;
  const { data: folders } = await supabase.from('folders').select('id, name').eq('owner_id', userId);
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const folderId = folderIds.length > 0 && Math.random() > 0.7 ? randomElement(folderIds) : null;
    contacts.push({ name, email, phone, company, role, description, tags, folder_id: folderId, owner_id: userId, company_id: companyId, is_shared: false });
  }
  ...
  for (let i = 0; i < contacts.length; i += batchSize) {
    ...
    if (error) { console.error(`❌ Batch ${Math.floor(i / batchSize) + 1} error:`, error); throw error; }
